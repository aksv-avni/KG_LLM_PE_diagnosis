...
def visualize_kg(nx_graph, output_file="pe_pilot_graph.html"):
    ...
    print(f"✅ Graph saved to {os.path.abspath(output_file)}. Open this file in your browser.")
...
"""groq api key : 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"""
...
def visualize_and_save_kg(G, output_file="PE_Reasoning_KG.html"):
    ...
    print("✅ Structural data saved to: pe_pilot_data.gexf")
...
def export_triplets_to_csv(G, output_file="extracted_triplets_final.csv"):
    ...
    print(f"✅ Extracted {len(df_triplets)} triplets to {output_file}")
    return df_triplets
...
def analyze_kg_insights(G):
    ...
    print(f"\n✅ Total Positive Findings Extracted: {len(positive_cases)}")
...
def generate_reasoning_audit(G, sample_id):
    ...
    print(f"\n🔍 Reasoning Audit for Sample {sample_id}:")
...
def analyze_and_export_pilot_v2(G):
    print("📊 --- REVISED PILOT DATA ANALYSIS --- 📊")
    ...
    print(f"\n✅ Diagnostic Assertions (Fuzzy Match):")
    for status, count in status_counts.items():
        print(f" - {status}: {count} triplets")
...
def visualize_subset(sub_G, output_file="debug_subset_view2.html"):
    ...
    net.toggle_physics(True)
    net.write_html(output_file)
    print(f"✅ Subset visualization saved as: {output_file}")
...
def export_subgraph_csv(full_graph, num_samples=6, output_name="subgraph_triplets.csv"):
    ...
    print(f"✅ Exported {len(df_sub)} triplets from {len(sample_ids)} samples to {output_name}")
    return df_sub
